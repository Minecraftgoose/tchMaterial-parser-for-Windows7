...
1. 本工具运行在 **Python 3.8**（Windows 7 能用的最后一个官方版本）。3.8 没有 PEP 604 的运行时支持，因此**所有模块顶部都必须有 `from __future__ import annotations`**，且不得在运行时（非注解位置）使用 `X | Y` 或 `list[str]` 等下标泛型。
   另外，3.11 及更早的版本不允许 f-string 中嵌套同类型引号，请使用下述格式：
...
2. 本工具在设计上**仅支持 Windows 7**。不要新增 `platform.system()` 判定、`sys.platform` 条件导入，
   也不要为 macOS / Linux 添加降级逻辑。修改后可用 `python tools/check_py38_compat.py src` 复验。
3. 确保代码**可维护性**：
